...
public:
  ...
  : Node("floor_map_switcher")
  {
    ...
    service_ = this->create_service<std_srvs::srv::SetBool>(
      "/switch_floor",
      std::bind(&FloorMapSwitcher::handle_request, this, std::placeholders::_1, std::placeholders::_2));

    RCLCPP_INFO(this->get_logger(),
                "FloorMapSwitcher ready. Call with: ros2 service call /switch_floor std_srvs/srv/SetBool \"{data: true}\"");
  }

private:
  void handle_request(const std_srvs::srv::SetBool::Request::SharedPtr request,
                      std_srvs::srv::SetBool::Response::SharedPtr response)
  {
    bool use_floor1 = request->data;
    std::string map_yaml = map_paths_[use_floor1];
    RCLCPP_INFO(this->get_logger(), "Switching to map: %s", map_yaml.c_str());

    if (!wait_for_service(map_client_, "map_server/load_map") ||
        !wait_for_service(clear_global_client_, "global_costmap") ||
        !wait_for_service(clear_local_client_, "local_costmap"))
    {
      response->success = false;
      response->message = "Service(s) not available.";
      return;
    }
    ...
    {
      response->success = false;
      response->message = "LoadMap service call failed.";
      return;
    }
    ...
    if (result->result != nav2_msgs::srv::LoadMap::Response::RESULT_SUCCESS) {
      response->success = false;
      response->message = "LoadMap returned error code " + std::to_string(result->result);
      return;
    }
    ...
    response->message = "Switched to " + map_yaml;
  }

  bool wait_for_service(rclcpp::ClientBase::SharedPtr client, const std::string & name)
  {
                     ...
                     const std::string & name)
  {
    ...
    if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), future)
      ...
      RCLCPP_WARN(this->get_logger(), "Failed to clear %s", name.c_str());
    }
  }
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<FloorMapSwitcher>());
  rclcpp::shutdown();
  return 0;
}
